...
flags.DEFINE_string('job_dir', 'logs', 'Where to save training models')
...
def prepare_dirs(config, prefix=['HMR']):
    ...
    if config.load_path:
        ...
        config.model_dir = osp.join(config.job_dir, save_name)

    for path in [config.job_dir, config.model_dir]:
